...
def overlay_image_alpha(img, img_overlay, x, y):
    """Overlay img_overlay on top of img at the position (x, y) blending with alpha channel."""
    if img_overlay is None or len(img_overlay.shape) < 3: return
...
def draw_shortcut_cell(
    ...
    # Check for icon
    icon_size = 50
    safe_name = text.replace("'", "").replace(" ", "_").lower()
    img_path = os.path.join("icons", f"{safe_name}.png")
    
    icon_img = ICONS_CACHE.get(safe_name)
    if icon_img is None and os.path.exists(img_path):
        icon_orig = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)
        if icon_orig is not None:
            icon_img = cv2.resize(icon_orig, (icon_size, icon_size))
            ICONS_CACHE[safe_name] = icon_img

    if icon_img is not None:
        icon_x = x + (box_w - icon_size) // 2
        icon_y = y + 15
        overlay_image_alpha(keyboard, icon_img, icon_x, icon_y)
        # Shift text downwards to fit below icon
        center_pt = (x + box_w // 2, y + box_h - 25)
        text_y = y + box_h - 15  # For english
    else:
        # Default centering if no icon
