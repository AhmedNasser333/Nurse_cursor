...
ENGLISH_LAYOUT = [
    ...
    ["M", "N", "O", "P", "Q", "R"],
    ["S", "T", "U", "V", "W", "X"],
    ["Y", "Z", "SPACE", "ENTER", "DEL", "BACK", "CLEAR"],
]
...
class KeyboardState:
    ...
    col_index: int = 0
    typed_text: str = ""
    displayed_text: str = ""   # text confirmed to screen via ENTER
...
# Return codes from on_confirm_key
CONFIRM_NORMAL = 0
CONFIRM_BACK = 1       # BACK key -> go to home menu
CONFIRM_ENTER = 2      # ENTER key -> commit text to display + play bell
CONFIRM_CLEAR = 3      # CLEAR key -> clear all typed text

def on_confirm_key(state: KeyboardState) -> int:
    """Row->col, then type key.
    Returns:
        CONFIRM_BACK  (1) if BACK chosen
        CONFIRM_ENTER (2) if ENTER chosen
        CONFIRM_CLEAR (3) if CLEAR chosen
        CONFIRM_NORMAL(0) otherwise
    """
    layout = get_current_layout()
    if state.scan_mode == "row":
        state.scan_mode = "col"
    ...
    result = CONFIRM_NORMAL
    if state.row_index < len(layout) and state.col_index < len(layout[state.row_index]):
        k = layout[state.row_index][state.col_index]
        if k == "SPACE":
            state.typed_text += " "
        elif k == "ENTER":
            # Commit current typed_text to displayed_text
            state.displayed_text = state.typed_text
            state.typed_text = ""
            result = CONFIRM_ENTER
        elif k == "DEL":
